...
import { Bell, ChevronDown, UserLock, X } from "lucide-react";
...
import { useGetLabelDetails } from "@/util/customHooks/useQueries";
...
export default function Page({
  ...
  const {
    ...
    isRefetching,
    isError,
  } = useGetLabelDetails({ labelId });
  const handleSubmitNotificationForm = async () => {
    if (!labelId) {
      return toast.warn("No label Details");
    }

    try {
      setisSubmitting(true);
      const res = await api.post(
        "admin/artist/all-labels/" + labelId,
        notifyUserForm,
      );
  ...
  const handleSubmitDeactivationForm = async () => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({
        queryKey: ["admin-all-labels ", "inactive"],
      });
    } catch (error) {
      if (isAxiosError(error)) {
        return;
  ...
  return (
    <div className="bg-main-white min-h-screen w-full flex flex-col lg:pl-[260px] px-5 overflow-hidden">
      ...
      {isLoadinglabelDetails || !labelDetails ? (
        <InlineLoadingScreen />
      ) : (
        <>
          ...
          <div className="flex gap-5">
            <div className="w-full">
              ...
              <div
                className={
                  "bg-secondary-50 p-3 rounded-lg flex-1 h-fit " +
                  (isLoadinglabelDetails && " shimmer")
                }
              >
                <div
                  className={
                    " flex flex-col justify-between h-full " +
                    ((isLoadinglabelDetails || !labelDetails) && "hidden")
                  }
                >
            ...
            <div className="w-full">
              ...
              <div
                className={
                  "bg-primary-50 p-3 rounded-lg flex-1 h-fit " +
                  (isLoadinglabelDetails && " shimmer")
                }
              >
                <div
                ...
                >
                  <div className="flex gap-3">
                    <div className="relative w-10 h-10 max-w-10 max-h-10">
                      <Image
                        priority={true}
                        src={labelDetails.data.labelLogo}
                        alt="artist Image"
                        fill
                        className="rounded-lg object-cover"
                      />
                    ...
                    <div className="flex flex-col">
                      <p className="text-md font-bold leading-[20px] tracking-tighter text-black">
                        {labelDetails.data.user.lastName +
                          " " +
                          labelDetails.data.user.firstName}
                      </p>
                      <p className="text-black text-sm font-light leading-[18px] tracking-tighter">
                        {labelDetails.data.user.email}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="flex flex-col gap-3">
            <p className="font-bold text-sm text-primary-500 mt-15">
              {labelDetails.data.artists.length} Artists
            </p>
            <div className="bg-neutral-50 border border-neutral-100 rounded-lg h-[300px] overflow-y-scroll">
              {labelDetails.data.artists.map((item, index) => (
                <Link
                  href={
                    "/dashboardAdmin/artist/all-artists/" +
                    item._id +
                    "?tab=artist-info"
                  }
                  key={index}
