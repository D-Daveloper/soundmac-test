...
import { usePaginatedAdminAllVerificationRequests } from "@/util/customHooks/useQueries";
...
import AllVerificationReqeuestTable from "./AllVerificationReqeuestTable";
...
const Page = () => {
  ...
  } = usePaginatedAdminAllVerificationRequests({
    ...filter,
    sort: "",
    page,
    limit: "50",
          ...
          {!allUsers || allUsers.data.length < 1 ? (
            <div className="flex flex-col justify-center items-center h-[80dvh] gap-15 ">
              ...
              <p className="text-text-body font-normal leading-[18px] tracking-[-0.5px] text-[16px] sm:max-w-[40%] text-center">
                There are no Verification Requests right now.
              </p>
            </div>
          ) : (
            <div className="mt-5 flex flex-col mb-10">
              <AllVerificationReqeuestTable
                users={allUsers.data}
                isfetching={isFetchingallUsers}
              />
              {/* Pagination */}
