  ...
  try {
    ...
    if (!file) {
      ...
      return NextResponse.json(
        { msg: "Please indicate the source of the report" },
        { status: 400 },
      );
    } else if (!sales_period || typeof sales_period != "string") {
      return NextResponse.json(
        { msg: "Please indicate the sales period" },
        { status: 400 },
      );
    } else if (!accounting_period || typeof accounting_period != "string") {
      return NextResponse.json(
        { msg: "Please indicate the upload date" },
        { status: 400 },
      );
    }
    await dbConnect();
    ...
    const batch: any = await salesReportBatch
      .findOne({ fileName: file.name })
      .lean();
    if (batch && batch.status === "processing") {
      return NextResponse.json(
        { msg: "Sales report is being processed. Should be done shortly." },
        { status: 400 },
      );
    } else if (batch && batch.status === "completed") {
      return NextResponse.json(
        { msg: "Sales report has been uploaded" },
        { status: 400 },
      );
    }
    ...
    const newBatch = await salesReportBatch.findOneAndUpdate(
      { fileName: file.name },
      {
        fileName: file.name,
    ...
    await logAdminActivity({
      ...
      action: "salesReport.uploaded",
      entityType: "sales Report",
      entityId: newBatch._id,
      entityLabel: file.name,
      metadata: {
        royaltySource: royalty_source,
