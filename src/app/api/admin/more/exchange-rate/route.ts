...
export async function GET() {
    try {
        ...
        const rate = await Counter.findById("exchange_rate").select("value -_id").lean();
        console.log(rate);
...
export async function POST(req: Request) {
    try {
        ...
        } else if (!numRegex.test(body.exchange_rate) ||
            parseInt(body.exchange_rate, 10) <= 0) {
            return NextResponse.json({ msg: "Please enter a valid number." }, { status: 400 })
        }

        await Counter.findByIdAndUpdate("exchange_rate", { value: body.exchange_rate?.trim() })
