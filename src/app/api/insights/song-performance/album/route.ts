...
export async function GET(req: Request) {
    try {
        ...
        if (albumTitle) {
            matchStage.trackTitle = { $regex: albumTitle, $options: 'i' };
        }
        ...
        const results = await salesReport.aggregate([
            ...
            {
                $lookup: {
                    from: 'albums', // Your song collection name
                    localField: 'upc',
                    foreignField: 'upc',
                    as: 'albumDetails'
                }
            ...
            // Project final shape
            {
                $project: {
                    ...
                    trackTitle: '$albumDetails.releaseTitle',
                    trackArtists: '$albumDetails.artistName',
                    releaseDate: 1,
                    totalStreams: 1,
                    totalDownloads: 1,
                    totalLikes: 1,
                    upc: 1,
                    releaseImage: '$albumDetails.releaseImage', // Adjust field name as needed
        ...
        return NextResponse.json(
            { success: false, error: 'Failed to report' },
            { status: 500 }
        );
    }
}