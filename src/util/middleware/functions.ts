...
export const isFormValid = (form:SongForm):string => {
      console.log(form);
  ...
  if (form.song_title === "") {
      ...
      } else if (form.song_title.length < 3 || form.song_title.length > 32) {
        ...
        return"featured artist name is required";
      } else if (
        ...
        return"lyrics is required";
      } else if (form.start_clip == "" || !parseFloat(form.start_clip)) {
        return"Starting Clip is required and must be a valid number";
      } else if (form.song_image == null) {
        return"Image is required";
      } else if (form.another_distribution_check && (form.isrc === "" || form.upc === "")) {
        return"ISRC and UPC is required";
      } else if (form.copyRightHolder === "" || form.copyRightYear === undefined ) {
